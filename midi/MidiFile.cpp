...
MidiFile::MidiFile(const MidiFile& other)
{
    ...
    std::generate_n(std::back_inserter(events), other.events.size(),
        [&]() -> MidiEventList* { return new MidiEventList(**it++); });
...
int MidiFile::read(istream& input)
{
    ...
    if(input.peek() != 'M')
    {
        ...
        // the MIDI file is in the binasc format which is an ASCII
        // representation of the MIDI file.  Convert the binasc content into
        // binary content and then continue reading with this function.
    ...
    else if(character != 'M')
    {
        ...
        cerr << "Expecting 'M' at first byte but got '" << (char)character
             << "'" << endl;
        rwstatus = 0;
        return rwstatus;
    }
    ...
    {
        cerr << "File " << filename << " is not a MIDI file" << endl;
        cerr << "Expecting 'T' at second byte but got '" << (char)character
             << "'" << endl;
        rwstatus = 0;
        return rwstatus;
    }
    ...
    {
        cerr << "File " << filename << " is not a MIDI file" << endl;
        cerr << "Expecting 'h' at third byte but got '" << (char)character
             << "'" << endl;
        rwstatus = 0;
        return rwstatus;
    }
    ...
    {
        cerr << "File " << filename << " is not a MIDI file" << endl;
        cerr << "Expecting 'd' at fourth byte but got '" << (char)character
             << "'" << endl;
        rwstatus = 0;
        return rwstatus;
    }
    ...
    if(longdata != 6)
    {
        cerr << "File " << filename << " is not a MIDI 1.0 Standard MIDI file."
             << endl;
    ...
    switch(shortdata)
    {
        ...
        default:
            cerr << "Error: cannot handle a type-" << shortdata << " MIDI file"
                 << endl;
            rwstatus = 0;
            return rwstatus;
    }
    ...
    if(shortdata >= 0x8000)
    {
        ...
        // cerr << "SMPTE ticks: " << ticksPerQuarterNote << " ticks/sec" <<
        // endl; cerr << "SMPTE frames per second: " << framespersecond << endl;
    ...
    for(int i = 0; i < tracks; i++)
    {
        ...
        if(character == EOF)
        {
            cerr << "In file " << filename << ": unexpected end of file."
                 << endl;
        ...
        while(!input.eof())
        {
            ...
            if(bytes[0] == 0xff && (bytes[1] == 1 || bytes[1] == 2 ||
                                       bytes[1] == 3 || bytes[1] == 4))
            {
...
int MidiFile::write(ostream& out)
{
    ...
    // 5. write out the number of ticks per quarternote. (avoiding SMTPE for
    // now)
    ...
    for(i = 0; i < getNumTracks(); i++)
    {
        ...
        for(j = 0; j < (int)events[i]->size(); j++)
        {
            ...
            {
                // 0xf0 == Complete sysex message (0xf0 is part of the raw
                // MIDI). 0xf7 == Raw byte message (0xf7 not part of the raw
                // MIDI). Print the first byte of the message (0xf0 or 0xf7),
                // then print a VLV length for the rest of the bytes in the
                // message. In other words, when creating a 0xf0 or 0xf7 MIDI
                // message, do not insert the VLV byte length yourself, as this
                // code will do it for you automatically.
        ...
        if((size < 3) ||
            !((trackdata[size - 3] == 0xff) && (trackdata[size - 2] == 0x2f)))
        {
...
void MidiFile::deltaTicks(void)
{
    ...
    for(i = 0; i < length; i++)
    {
        ...
        for(j = 1; j < (int)events[i]->size(); j++)
        {
            ...
            if(deltatick < 0)
            {
                cerr << "Error: negative delta tick value: " << deltatick
                     << endl
...
double MidiFile::getTimeInSeconds(int tickvalue)
{
    ...
    void* ptr = bsearch(
        &key, timemap.data(), timemap.size(), sizeof(_TickTime), ticksearch);
...
int MidiFile::getAbsoluteTickTime(double starttime)
{
    ...
    void* ptr = bsearch(
        &key, timemap.data(), timemap.size(), sizeof(_TickTime), secondsearch);
...
MidiEvent* MidiFile::addMetaEvent(
    int aTrack, int aTick, int aType, vector<uchar>& metaData)
{
    timemapvalid = 0;
    int i;
...
MidiEvent* MidiFile::addMetaEvent(
    int aTrack, int aTick, int aType, const char* metaData)
{
...
MidiEvent* MidiFile::addInstrumentName(
    int aTrack, int aTick, const string& name)
{
    MidiEvent* me = new MidiEvent;
...
MidiEvent* MidiFile::addTimeSignature(int aTrack, int aTick, int top,
    ...
    int bottom, int clocksPerClick, int num32ndsPerQuarter)
{
    return addTimeSignature(
        aTrack, aTick, top, bottom, clocksPerClick, num32ndsPerQuarter);
}
...
MidiEvent* MidiFile::addNoteOn(
    int aTrack, int aTick, int aChannel, int key, int vel)
{
    MidiEvent* me = new MidiEvent;
...
MidiEvent* MidiFile::addController(
    int aTrack, int aTick, int aChannel, int num, int value)
{
    MidiEvent* me = new MidiEvent;
...
MidiEvent* MidiFile::addPatchChange(
    int aTrack, int aTick, int aChannel, int patchnum)
{
    MidiEvent* me = new MidiEvent;
...
MidiEvent* MidiFile::addPitchBend(
    int aTrack, int aTick, int aChannel, double amount)
{
    timemapvalid = 0;
    amount += 1.0;
...
//////////////////////////////
//
// MidiFile::addSustain -- Add a continuous controller message for the sustain
// pedal.
//
...
MidiEvent* MidiFile::addSustainPedal(
    int aTrack, int aTick, int aChannel, int value)
{
...
//////////////////////////////
//
// MidiFile::addSustainOn -- Add a continuous controller message for the sustain
// pedal on.
//
...
//////////////////////////////
//
// MidiFile::addSustainOff -- Add a continuous controller message for the
// sustain pedal off.
//
...
void MidiFile::buildTimeMap(void)
{
    ...
    for(i = 0; i < getNumEvents(0); i++)
    {
        ...
        {
            secondsPerTick =
                getEvent(0, i).getTempoSPT(getTicksPerQuarterNote());
        }
    }
...
int MidiFile::extractMidiData(
    istream& input, vector<uchar>& array, uchar& runningCommand)
{
    ...
    switch(runningCommand & 0xf0)
    {
        ...
        case 0xF0:
            switch(runningCommand)
            {
                    ...
                    if(byte1 >= 0x80)
                    {
                        ...
                        if(byte2 > 0x80)
                        {
                            ...
                            if(byte3 >= 0x80)
                            {
                                ...
                                if(byte4 >= 0x80)
                                {
                                    cerr << "Error: cannot handle large VLVs"
                                         << endl;
                                    rwstatus = 0;
                                    return rwstatus;
                                }
                                ...
                                {
                                    length =
                                        unpackVLV(byte1, byte2, byte3, byte4);
                                    if(!rwstatus)
                                    {
                                        return rwstatus;
                                    }
                ...
                // typically the middle or ends of system exclusive messages.
                // The 0xf7 byte at the start of the message is not part of the
