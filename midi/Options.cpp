...
// Web Address:
// https://github.com/craigsapp/optionlib/blob/master/include/Options.h
...
Option_register::Option_register(
    const string& aDefinition, char aType, const string& aDefaultOption)
{
    type = 's';
    modifiedQ = 0;
    setType(aType);
...
int Options::define(const string& aDefinition)
{
    ...
    if(otype[0] != OPTION_STRING_TYPE && otype[0] != OPTION_INT_TYPE &&
        otype[0] != OPTION_FLOAT_TYPE && otype[0] != OPTION_DOUBLE_TYPE &&
        otype[0] != OPTION_BOOLEAN_TYPE && otype[0] != OPTION_CHAR_TYPE)
    {
    ...
    for(i = 0; i < aliases.size(); i++)
    {
        ...
        else if(aliases[i] == '|')
        {
            ...
            {
                cerr << "Option \"" << optionName
                     << "\" from definition:" << endl;
...
int Options::storeOption(int gargp, int& position, int& running)
{
    ...
    switch(optionForm)
    {
        ...
        case OPTION_FORM_LONG:
            position = 2;
            while(
                oargv[gargp][position] != '=' && oargv[gargp][position] != '\0')
            {
    ...
    if(oargv[gargp][position] == '\0' && optionType != OPTION_BOOLEAN_TYPE)
    {
        gargp++;
        position = 0;
    }
...
ostream& Options::printOptionListBooleanState(ostream& out)
{
    for(auto it = optionList.begin(); it != optionList.end(); it++)
    {
        out << it->first << "\t" << optionRegister[it->second]->isModified()
            << endl;
    }
    return out;
}
