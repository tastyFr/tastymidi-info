...
int Binasc::outputStyleAscii(ostream& out, istream& input)
{
    ...
    while(!input.eof())
    {
        ...
        if((type == 1) && (lastType == 0))
        {
            ...
            if(index + lineCount >= maxLineLength)
            { // put on next line
                outputWord[index] = '\0';
                out << '\n' << outputWord;
                lineCount = index;
...
int Binasc::getWord(
    string& word, const string& input, const string& terminators, int index)
{
    word.resize(0);
    int i = index;
    int escape = 0;
...
int Binasc::readMidiEvent(
    ostream& out, istream& infile, int& trackbytes, int& command)
{
...
int Binasc::outputStyleMidi(ostream& out, istream& input)
{
    ...
    if(ch != 'M')
    {
        cerr << "Not a MIDI file M" << endl;
        return 0;
    }
    input.read((char*)&ch, 1);
    if(ch != 'T')
    {
        cerr << "Not a MIDI file T" << endl;
        return 0;
    }
    input.read((char*)&ch, 1);
    if(ch != 'h')
    {
        cerr << "Not a MIDI file h" << endl;
        return 0;
    }
    input.read((char*)&ch, 1);
    if(ch != 'd')
    {
        cerr << "Not a MIDI file d" << endl;
        return 0;
    }
    tempout << "\"MThd\"";
    if(commentsQ)
    ...
    for(i = 0; i < trackcount; i++)
    {
        ...
        if(ch != 'M')
        {
            cerr << "Not a MIDI file M2" << endl;
            return 0;
        }
        input.read((char*)&ch, 1);
        if(ch != 'T')
        {
            cerr << "Not a MIDI file T2" << endl;
            return 0;
        }
        input.read((char*)&ch, 1);
        if(ch != 'r')
        {
            cerr << "Not a MIDI file r" << endl;
            return 0;
        }
        input.read((char*)&ch, 1);
        if(ch != 'k')
        {
            cerr << "Not a MIDI file k" << endl;
            return 0;
        }
        tempout << "\"MTrk\"";
        if(commentsQ)
...
int Binasc::processDecimalWord(ostream& out, const string& word, int lineNum)
{
        ...
        switch(word[i])
        {
            ...
            case '-':
                ...
                if(i == 0 || word[i - 1] != '\'')
                {
                    cerr << "Error on line " << lineNum << " at token: " << word
                         << endl;
                    cerr << "minus sign must immediately follow quote mark"
                         << endl;
                    return 0;
                }
                break;
    ...
    if(periodIndex != -1)
    {
        ...
        switch(byteCount)
        {
            ...
            default:
                cerr << "Error on line " << lineNum << " at token: " << word
                     << endl;
                cerr << "floating-point numbers can be only 4 or 8 bytes"
                     << endl;
                return 0;
        }
    }
    ...
    switch(byteCount)
    {
        ...
        default:
            cerr << "Error on line " << lineNum << " at token: " << word
                 << endl;
            cerr << "invalid byte count specification for decimal number"
                 << endl;
            return 0;
    }
...
int Binasc::processMidiTempoWord(ostream& out, const string& word, int lineNum)
{
    if(word.size() < 2)
    {
        cerr << "Error on line: " << lineNum
...
int Binasc::processMidiPitchBendWord(
    ostream& out, const string& word, int lineNum)
{
    if(word.size() < 2)
    {
        cerr << "Error on line: " << lineNum
