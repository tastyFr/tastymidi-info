...
MidiEventList::MidiEventList(const MidiEventList& other)
{
    ...
    std::generate_n(std::back_inserter(list), other.list.size(),
        [&]() -> MidiEvent* { return new MidiEvent(**it++); });
}
    ...
    for(i = 0; i < getSize(); i++)
    {
        ...
        else if(mev->isController())
        {
            ...
            if(contmap[contnum].first)
            {
                ...
                {
                    // controller is currently off, so store on-state for next
                    // link
                    contevents[conti][channel] = mev;
                    oldstates[conti][channel] = contstate;
                }
...
int eventcompare(const void* a, const void* b)
{
    ...
    else if(((aevent.getP0() & 0xf0) == 0x90) ||
            ((aevent.getP0() & 0xf0) == 0x80))
    {
        // note-offs come after all other MIDI messages (except note-ons)
        return +1;
    }
    else if(((bevent.getP0() & 0xf0) == 0x90) ||
            ((bevent.getP0() & 0xf0) == 0x80))
    {
        // note-offs come after all other MIDI messages (except note-ons)
        return -1;
    }
    else if(((aevent.getP0() & 0xf0) == 0xb0) &&
            ((bevent.getP0() & 0xf0) == 0xb0))
    {
        // both events are continuous controllers.  Sort them by controller
        // number
